...
def search_locations_by_product_type():
    ...
    locations = search_service.get_locations_by_product_type(
...
def get_area_info():
    ...
    logger.info(
        f"get custmers around location ({x}, {y}), customers count: {len(customers)}")
...
def realtime_chart():
    ...
    if product_type:
        logger.info(
            f"get real time transactions for product_type: {product_type}")
    if loc:
        logger.info(f"get real time transactions for location: {loc}")
    xdata, ydata = Realtime_Monitor.get_current_data(
        product_type=product_type, radius=float(radius), loc=loc)
